...
static int tja1102_phy_fixup(struct phy_device *phydev)
{
...
#if 0
	err = phy_configure_bit(phydev, MII_CONFIGURATION_3, COMMCFG_AUTO_OP, 1);
	if (err < 0)
		printk("Harry:************ auto_op mode failed!\n");
	reg_value = phy_read(phydev, MII_CONFIGURATION_3);
	printk("Harry:*******MII_CONFIGURATION_3(27) reg_value is %8x  \n", reg_value);
	if (reg_value < 0)
		printk("Harry:************read register failed!");

	/* Master slave configuration */
	err = phy_configure_bit(phydev, MII_CFG1, CFG1_MASTER_SLAVE, 1);
	if (err < 0)
		printk("Harry:************  Master slave failed!\n");
#endif
#if 1
	...
	return 0;
	//phy_configure_error:
	//printk("Harry******phy r/w error: wakeup failed\n");
	//return err;
	//phy_transition_error:
	//printk("Harry******power mode transition failed\n");
	//return err;
	
#if 0
	reg_value = phy_read(phydev, 0);
	printk("Harry:****Register(0) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, 1);
	printk("Harry:****Register(1) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, 0x0F);
	printk("Harry:****Register(15) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, MII_CFG2);
	printk("Harry:******MII_CFG2(18) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, MII_COMMSTAT);
	printk("Harry:*******MII_COMMSTAT(22) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, MII_GENSTAT);
	printk("Harry:*******MII_GENSTAT(23) reg_value is %8x  \n", reg_value);
	reg_value = phy_read(phydev, MII_CONFIGURATION_3);
	printk("Harry:*******MII_CONFIGURATION_3(27) reg_value is %8x  \n", reg_value);
	if (reg_value < 0)
		printk("Harry:************read register failed!");
...
static int tja1100_phy_fixup(struct phy_device *phydev)
{
	u16 val;
	int err;
	int reg_name, reg_value = -1, reg_mask;
	...
	err = phy_configure_bit(phydev, MII_CFG1, CFG1_MASTER_SLAVE, 1);
	if (err < 0)
		printk("Harry:************enable master mode failed!");
	...
	if (err < 0)
		printk("Harry:************clear BMCR_PDOWN failed!");
	...
	err = phy_configure_bits(phydev, MII_ECTRL, ECTRL_POWER_MODE,
				 POWER_MODE_NORMAL);
	if (err < 0)
		printk("Harry:************transit to normal mode failed!");

	/* wait until power mode transition is completed */
	err = wait_on_condition(phydev, MII_ECTRL, ECTRL_POWER_MODE,
				POWER_MODE_NORMAL, POWER_MODE_TIMEOUT);
	if (err < 0)
		printk("Harry:************wait until failed!");

	/* wait until the PLL is locked, indicating a completed transition */
	err = wait_on_condition(phydev, MII_GENSTAT, GENSTAT_PLL_LOCKED,
				GENSTAT_PLL_LOCKED, POWER_MODE_TIMEOUT);
	if (err < 0)
		printk("Harry:************wait until PLL is locked failed!");
#endif	
	/* reenable link control */
	set_link_control(phydev, 1);
	#endif
