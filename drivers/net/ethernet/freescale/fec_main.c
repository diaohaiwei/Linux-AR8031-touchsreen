...
fec_restart(struct net_device *ndev, int duplex)
{
	...
	if (id_entry->driver_data & FEC_QUIRK_ENET_MAC) {
		...
		{
			rcntl |= (1 << 6);
			printk("Harry:**********: fep->phy_interface == PHY_INTERFACE_MODE_RGMII!\n");
		}
		else if (fep->phy_interface == PHY_INTERFACE_MODE_RMII)
		{
			rcntl |= (1 << 8);
			printk("Harry:**********: fep->phy_interface == PHY_INTERFACE_MODE_RMII!\n");
		}
		else
			rcntl &= ~(1 << 8);
		...
		while (!((status = bdp->cbd_sc) & BD_ENET_RX_EMPTY)) {
			...
			if (status & (BD_ENET_RX_LG | BD_ENET_RX_SH | BD_ENET_RX_NO |
				...
				if (status & (BD_ENET_RX_LG | BD_ENET_RX_SH)) {
					...
					printk("Harry:************Frame too long or too short!\n");
				}
				if (status & BD_ENET_RX_NO){
					/* Frame alignment */
					ndev->stats.rx_frame_errors++;
					printk("Harry:************Frame alignment error!\n");
					}
				if (status & BD_ENET_RX_CR){
					/* CRC Error */
					ndev->stats.rx_crc_errors++;
					printk("Harry:************CRC Error!\n");
					}
				if (status & BD_ENET_RX_OV){
					/* FIFO overrun */
					ndev->stats.rx_fifo_errors++;
					printk("Harry:************FIFO overrun Error!\n");
					}
			}
			...
			if (status & BD_ENET_RX_CL) {
				ndev->stats.rx_errors++;
				ndev->stats.rx_frame_errors++;
				printk("Harry:************drop this frame on the floor Error!\n");
				goto rx_processing_done;
			}
...
static bool fec_enet_collect_events(struct fec_enet_private *fep)
{
	...
	if (int_events & FEC_ENET_MII)
		fep->work_mdio = 1;
    //printk("Harry:**********: fec_enet_collect:int_events is %x!\n", int_events );
...
{
	struct fec_enet_private *fep = netdev_priv(ndev);
	printk("Harry:**********: fec_enet_clk_enable!\n");
	if (enable) {
...
static int fec_enet_mii_probe(struct net_device *ndev)
{
	...
	printk("Harry:**********: fec_enet_mii_probe!\n");
	fep->phy_dev = NULL;
...
static int fec_enet_mii_init(struct platform_device *pdev)
{
	...
	const struct platform_device_id *id_entry =
				platform_get_device_id(fep->pdev);
	int err = -ENXIO, i;
	printk("Harry:**********: fec_enet_mii_init!\n");
	/*
