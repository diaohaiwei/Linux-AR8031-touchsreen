...
MODULE_LICENSE("GPL");
extern int tja1100_phy_fixup(struct phy_device *phydev);
...
{
	//printk("Harry:************fixup->bus_id = %8x, dev_name(&phydev->dev) = %8x!\n", fixup->bus_id, dev_name(&phydev->dev));
	//printk("Harry:************PHY_ANY_ID = %8x!\n", PHY_ANY_ID);
	//printk("Harry:************fixup->phy_uid  = %8x, fixup->phy_uid_mask = %8x\n", fixup->phy_uid, fixup->phy_uid_mask);
	//printk("Harry:************phydev->phy_id  = %8x, fixup->phy_uid_mask = %8x\n", phydev->phy_id, fixup->phy_uid_mask);
	printk("Harry:************phydev->phy_id  = %8x !\n",phydev->phy_id);
...
{
	struct phy_fixup *fixup;
	printk("Harry:************phy_scan_fixups\n");
	mutex_lock(&phy_fixup_lock);
	list_for_each_entry(fixup, &phy_fixup_list, list) {
		if (phy_needs_fixup(phydev, fixup)) {
			int err;
			printk("Harry:************fixup->run(phydev)\n");
...
int phy_device_register(struct phy_device *phydev)
{
	...
	printk("Harry:************phy_device_register:phy_scan_fixups !\n");
...
int phy_init_hw(struct phy_device *phydev)
{
    ...
    printk("Harry:*****phy_init_hw:phy_scan_fixups\n");
...
int genphy_update_link(struct phy_device *phydev)
{
    ...
    if(genphy_init_count <= 3){
        tja1100_phy_fixup(phydev);
        genphy_init_count++;
    }
	//printk("Harry:****genphy_update_link  \n");
	/* Do a fake read */
...
int genphy_read_status(struct phy_device *phydev)
{
	...
	if (AUTONEG_ENABLE == phydev->autoneg) {
		//if (AUTONEG_ENABLE != phydev->autoneg) {
		printk("Harry:************genphy phydev->autoneg! \n");
		if (phydev->supported & (SUPPORTED_1000baseT_Half
					| SUPPORTED_1000baseT_Full)) {
		...
		//printk("Harry:****genphy MII_BMCR(0) reg_value is %8x \n", bmcr);
		if (bmcr < 0)
			return bmcr;
...
static int genphy_config_init(struct phy_device *phydev)
{
	...
	features = (SUPPORTED_TP | SUPPORTED_MII
			...
			SUPPORTED_BNC);
	printk("Harry:************genphy_config_init\n");
	...
	if (val & BMSR_ESTATEN) {
		...
		if (val < 0)
			return val;
		printk("Harry:****genphy MII_ESTATUS(15) is %8x \n", val);
